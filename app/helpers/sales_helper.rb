module SalesHelper
  ...
  def update_clients qrcode_image_tempfile, sale_sum
  	...
  	@client = Client.find(qrcode_info.to_i)
  	@client.bonus_sum += (sale_sum * 0.01).round
  	@client.save
  ...
  def new_qrcode
		...
		image = MiniMagick::Image.read(qrcode.to_blob).format("jpg")
		image_location = "#{Rails.root}/tmp/#{@client.id}.jpg"		
		image.write(image_location)

		return image_location.gsub("#{Rails.root}/", root_path)
